...
function Tutor() {
  ...
  const activeFullPath = location.pathname.split("/").slice(2).join("/");
  ...
  return (
    <div className="w-full flex">
      ...
      >
        ...
        {!sideBar && <p className="font-semibold items-start justify-start px-5 text-gray-700 flex cursor-pointer" onClick={() => navigate('dashboard')}>DashBoard</p>}
        {/* Sidebar Items */}
                  ...
                  {item.children?.map((sub, subIndex) => (
                    <li key={subIndex}>
                      <div
                        ...
                        className={`flex ${sideBar && "justify-end"} items-center gap-2 text-[14px] px-8 h-[35px] transition-all duration-300 cursor-pointer ${activeFullPath === `${item.path}/${sub.path}`
                          ? "text-blue-500"
                          : "text-gray-500"} hover:text-black hover:bg-gray-200 font-medium`}
                      >
                        {activeFullPath === `${item.path}/${sub.path}` && !sideBar && (
      ...
      {/* Mobile Sidebar */}
      {mobileMenu && (
        <div className="fixed inset-0 bg-white text-white flex flex-col p-6 z-50 transition-transform duration-300 ease-in-out transform origin-top scale-100 opacity-100 lg:hidden">
          {/* Close Button */}
          <img
